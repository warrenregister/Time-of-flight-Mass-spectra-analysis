...
import numpy as np
import itertools
from data_creation import get_isotope_data


class AtomicPatternRecognizerNoML:
    def __init__(self, path):
        ...
        for row in isotope_df.itertuples():
            name = row.Element
            abunds = row.IsotopeFrequencies
            masses = row.IsotopeMasses
            try:
    ...
    def find_atomic_patterns(self, masses, intensities, thresh=5,
                             nums=(0, .5, 1)):
        """
        ...
        thresh: threshold value for check_abundance_match
        nums: nums value for round_mz_ratio
        """
        ...
        for key in candidates.keys():
            ...
            match = None
            best_max_diff = thresh
            for comb in candidate_combs:
                ratios = self.get_abundance_ratios(comb, intensities)

                matched, diffs = self.check_abundance_match(ratios, abundances,
                                                            thresh)
                max_diff = max(diffs)
                if matched and max_diff < best_max_diff:
                    match = comb
    ...
    def get_abundance_ratios(indices, intensities):
        ...
        total = np.sum(counts)
        return [x / total * 100 for x in counts]
